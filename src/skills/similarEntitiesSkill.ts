 ...
 * @example
 * // Create the skill
 * const similarCompaniesSkill = new SimilarEntitiesSkill(
 *   context,
 *   state,
 *   ai,
 *   AIPrompts.Entity
 * );
 * // Find the entity
 * const entity = await similarCompaniesSkill.run("Microsoft");
 * if (entity) {
 ...
 * }
 */
export class SimilarEntitiesSkill extends BaseAISkill {
  constructor(
    context: TurnContext,
    state: ApplicationTurnState,
    planner: ActionPlanner<ApplicationTurnState>
  ) {
    super(context, state, planner, AIPrompts.SimilarCompanies);
  }
